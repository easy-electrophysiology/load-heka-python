...
def get_data_kind(byte, endian):
    ...
    *)
    """
    opts = dict(
        zip(["IsLittleEndian", "IsLeak", "IsVirtual", "IsImon", "IsVmon", "Clip"], byte_to_bools(byte, endian)[0:7])
    )
...
def byte_to_bools(byte, endian):
    ...
    bits = np.unpackbits(np.array(byte, dtype=np.uint8), bitorder=bitorder)
...
def get_ext_trigger_type(byte):
    return ["TrigNone", "TrigSeries", "TrigSweep", "TrigSweepNoLeak"][byte]


def get_ampl_mode_type(byte):
    return ["AnyAmplMode", "VCAmplMode", "CCAmplMode", "IDensityMode"][byte]


def get_auto_ranging_type(byte):
    return ["AutoRangingOff", "AutoRangingPeak", "AutoRangingMean", "AutoRangingRelSeg"][byte]


def get_adc_type(byte):
    return ["AdcOff", "Analog", "Digitals", "Digital", "AdcVirtual"][byte]


def get_leak_store_type(byte):
    return ["LNone", "LStoreAvg", "LStoreEach", "LNoStore"][byte]


def get_leak_hold_type(byte):
    return ["Labs", "Lrel", "LabsLH", "LrelLH"][byte]


def get_break_type(byte):
    return ["NoBreak", "BreakPos", "BreakNeg"][byte]


def get_leak_comp_type(byte):
    return ["LeakCompSoft", "LeakCompHard"][byte]


def get_seg_store_type(byte):
    return ["SegNoStore", "SegStore", "SegStoreStart", "SegStoreEnd"][byte]


def get_recording_mode(byte):

    try:
        mode = ["InOut", "OnCell", "OutOut", "WholeCell", "CClamp", "VClamp", "NoMode"][byte]
