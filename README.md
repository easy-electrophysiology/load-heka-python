...
`series["data"]` - a num sweeps x num samples numpy array containing the data. Note that in HEKA it is possible for
                   different sweeps in the same series to have a different number of samples. In this instance, the end
...
5) In general select 'Data' > 'Export Full Sweep' to ignore the way the trace is displayed on the graph (i.e. if it is zoomed in).
